...
# --- NEW: Checkpoint Utility Functions ---
def save_checkpoint(round_num, model_state, fedavg_acc_history, checkpoint_dir, filename='checkpoint.pt'):
    ...
    # Checkpoint klasörü yoksa oluştur
    os.makedirs(checkpoint_dir, exist_ok=True)
...
def load_checkpoint(model, checkpoint_dir, filename='checkpoint.pt'):
    ...
    if os.path.exists(filepath):
        ...
        # Determine starting round and load history
        start_round = checkpoint['round'] + 1
        fedavg_acc = checkpoint['fedavg_acc']
        ...
        return 1, []  # Start from round 1 with empty history
        ...
        for local_epoch in range(args.local_epochs):
            ...
            for epoch in range(local_iter):
                ...
                inputs = self.interleave(
                    ...
                    args.gpu_id)  # 将有标签和无标签数据交错组合成一个大的批次
                targets_x = targets_x.cuda(args.gpu_id)
                ...
                final_targets_u = torch.where(
                    ...
                    lambda_dynamic.unsqueeze(1) * targets_u_local_one_hot + (1 - lambda_dynamic).unsqueeze(
                        1) * targets_u_global_one_hot,  # 使用动态伪标签修正
                    targets_u_global_one_hot
                )
...
def fixmatch(alpha):
    ...
    # ----------------------------------------------------
    # 🌟 NEW: Load Checkpoint before starting the main loop
    # ----------------------------------------------------

    # Load model and get starting round and history
    start_round, fedavg_acc = load_checkpoint(global_model.model, checkpoint_dir)

    # If a checkpoint was loaded, the global_model is already updated.
    # If not, it's starting from round 1 with a freshly initialized model.
    ...
    # FL training loop is now adjusted to start from 'start_round'
    for r in tqdm(range(start_round, args.num_rounds + 1), desc='Server'):
        ...
        for client in online_clients:
            ...
            list_nums_local_data.append(len(data_client_labeled) + len(data_client_unlabeled))
            # Pass r (round number) to local training for logging purposes
        ...
        # 🌟 NEW: Update global model with fused parameters BEFORE evaluation/save
        ...
        # ----------------------------------------------------
        # 🌟 NEW: Save Checkpoint at the end of each round (r)
        # ----------------------------------------------------
        ...
        os.makedirs(result_dir, exist_ok=True)
        result_file = f'{result_dir}/SAGE α={alpha}.csv'
        # The accuracy history 'fedavg_acc' is updated, so this part remains the same.
        acc_num_pseudo_label_csv_index = list(range(1, len(fedavg_acc) + 1))
        acc_num_pseudo_label_csv_df = pd.DataFrame({'acc': fedavg_acc}, index=acc_num_pseudo_label_csv_index)
        # 保存文件
        acc_num_pseudo_label_csv_df.to_csv(result_file, encoding='utf8')
...
if __name__ == '__main__':
    ...
    # 🌟 To ensure `args.alpha` is available for checkpoint path:
    alpha_value = args.alpha
    fixmatch(alpha_value)
