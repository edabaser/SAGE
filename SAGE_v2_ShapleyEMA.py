"""
SAGE_v2_ShapleyEMA.py — SAGE + Shapley Weight EMA Smoothing

YENİLİK: Shapley aggregation ağırlıklarına EMA yumuşatma.
=========
Sorun   : Her round Shapley değerleri sıfırdan Monte Carlo ile hesaplanıyor.
          Erken roundlarda local modeller olgunlaşmamış → Shapley skorları
          gürültülü → ShapFed neredeyse FedAvg gibi davranıyor.
Çözüm   : Her client'ın Shapley skoru önceki round ile EMA ile yumuşatılır.
          shapley_ema[c] = 0.7 * shapley_ema[c] + 0.3 * cssv_current[c]
          Bu hem erken round gürültüsünü azaltır hem de geç roundlarda
          kararlı aggregation sağlar → ShapFed ≠ FedAvg farkı belirginleşir.

Yeni flag: --use_shapley_ema       (default False)
           --shapley_ema_decay 0.7 (EMA decay katsayısı)

Checkpoint adında "SEMA" görünür.

Kullanım:
  python SAGE_v2_ShapleyEMA.py --dataset HAM10000 --model resnet18 \
    --aggregation_method ShapFed --alpha 0.1 --num_labeled 100 \
    --use_shapley_ema --shapley_ema_decay 0.7 \
...
def args_parser():
    import argparse, sys
    parser = argparse.ArgumentParser(description='SAGE v2 - ShapleyEMA', add_help=False)
    parser.add_argument('--use_shapley_ema', action='store_true', default=False,
                        help='[YENİ] Shapley ağırlıklarına EMA yumuşatma')
    parser.add_argument('--shapley_ema_decay', type=float, default=0.7,
                        help='Shapley EMA decay (0.5-0.9 arası önerilir)')
...
def get_exp_name(args):
    ...
    if args.use_shapley_ema:      flags.append('SEMA')  # YENİ
...
class Global:
    def __init__(self, args):
        ...
        self.args = args
        # ── SHAPLEY EMA STATE (YENİ) ──────────────────────────
        # {client_id: np.array shape [num_classes]} — her client için EMA Shapley skoru
        # Yalnızca ShapFed + use_shapley_ema aktifken kullanılır.
            ...
            if args.use_shapley_ema and online_client_ids is not None:
                ...
                for i, cid in enumerate(online_client_ids):
                    ...
                    if key in self.shapley_ema_store:
                        ...
                        cssv[i] = cssv_raw[i]  # ilk round: EMA = raw
                    self.shapley_ema_store[key] = cssv[i].copy()
                # Sütun normalizasyonu yeniden uygula
...
def main_loop(alpha):
    ...
    print(f"  Shapley EMA   : {'✓' if args.use_shapley_ema else '✗'} (decay={args.shapley_ema_decay})")  # YENİ
    print(f"{'='*60}\n")
