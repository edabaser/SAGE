...
def _ham_weak_transform_standard():
    """Standart zayıf augmentation — ablation (use_medical_augment=False) için."""
    return transforms.Compose([
        transforms.Resize((224, 224)),
        transforms.RandomHorizontalFlip(),
...
def _ham_strong_transform():
    ...
    return transforms.Compose([
        ...
        transforms.RandomVerticalFlip(),
        transforms.RandomAffine(degrees=15, translate=(0.1, 0.1), scale=(0.9, 1.1)),
        RandAugmentMC(n=2, m=7),
        transforms.ToTensor(),
        transforms.Normalize(mean=HAM_MEAN, std=HAM_STD),
    ])


def _ham_strong_transform_standard():
    """Standart güçlü augmentation — ablation (use_medical_augment=False) için."""
    return transforms.Compose([
        transforms.Resize((224, 224)),
        transforms.RandomHorizontalFlip(),
        transforms.RandomCrop(size=224, padding=28, padding_mode='reflect'),
...
class Indices2Dataset_labeled(Dataset):
    """
    Labeled veri seti.
    use_medical_augment=True  → medikal-güvenli transform (renk bozmaz)
    use_medical_augment=False → standart FixMatch transform
    """

    def __init__(self, dataset, dataset_name='CIFAR10', use_medical_augment=True):
        self.dataset = dataset
        self.dataset_name = dataset_name
        self.use_medical_augment = use_medical_augment
        self.indices = None
    ...
    def __getitem__(self, idx):
        ...
        if self.dataset_name == 'HAM10000':
            trans = _ham_labeled_transform() if self.use_medical_augment else _ham_labeled_transform_standard()
        else:
            trans = _cifar_labeled_transform()
...
class Indices2Dataset_unlabeled_fixmatch(Dataset):
    """
    Unlabeled veri seti — zayıf + güçlü augmentation çifti döner.
    use_medical_augment=True  → medikal-güvenli augmentation
    use_medical_augment=False → standart FixMatch augmentation
    """
    ...
    def fixmatch(self, image):
        ...
        if self.dataset_name == 'HAM10000':
            if self.use_medical_augment:
                weak   = _ham_weak_transform()
                strong = _ham_strong_transform()
            else:
                # Standart FixMatch augmentation — renk bozabilir ama ablation için gerekli
                weak   = _ham_weak_transform_standard()
                strong = _ham_strong_transform_standard()
        else:
            weak   = _cifar_weak_transform()
            strong = _cifar_strong_transform()
