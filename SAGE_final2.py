...
def compute_cssv(args, local_models_params, initial_global_params, client_gradients=None):
    """
    ShapFed paper Equation 5 ile birebir uyumlu CSSV hesabı.

    Paper yöntemi (orijinal):
        Γᵢ,ⱼ = cos(ŵᵢ,ⱼ, ŵₛ,ⱼ)
        ŵᵢ,ⱼ: client i'nin son katman gradyenti, sınıf j sütunu
        ŵₛ,ⱼ: aggregated gradyent, sınıf j sütunu

    client_gradients: list of dicts — her client'ın gradient'i
        gradient = (w_before - w_after) / lr  (compute_full_grad)
    """
    num_clients = len(local_models_params)
    num_classes = args.num_classes
    if num_clients == 0:
    ...
    # Gradient veya weight delta seç
    if client_gradients is not None:
        # Paper orijinal: gerçek gradient kullan
        grads = []
        for g in client_gradients:
            grads.append({
                weight_layer: g[weight_layer].float().cpu(),
                bias_layer:   g[bias_layer].float().cpu(),
            })
    else:
        # Fallback: weight delta (local - global)
        g_w = initial_global_params[weight_layer].float().cpu()
        g_b = initial_global_params[bias_layer].float().cpu()
        grads = []
        for lp in local_models_params:
            grads.append({
                weight_layer: lp[weight_layer].float().cpu() - g_w,
                bias_layer:   lp[bias_layer].float().cpu()   - g_b,
            })

    # Aggregated gradient: uniform ortalama (ilk round için)
    agg = {
        weight_layer: sum(g[weight_layer] for g in grads) / num_clients,
        bias_layer:   sum(g[bias_layer]   for g in grads) / num_clients,
    }

    # Paper Equation 5: Γᵢ,ⱼ = cos(ŵᵢ,ⱼ, ŵₛ,ⱼ) ∈ [-1, 1]
    cssv = np.zeros((num_clients, num_classes))
    for i in range(num_clients):
        for c in range(num_classes):
            ci_vec = torch.cat([grads[i][weight_layer][c].view(-1),
                                grads[i][bias_layer][c].view(-1)])
            ag_vec = torch.cat([agg[weight_layer][c].view(-1),
                                agg[bias_layer][c].view(-1)])
            n_ci = torch.norm(ci_vec)
            n_ag = torch.norm(ag_vec)
            if n_ci > 1e-8 and n_ag > 1e-8:
                cssv[i, c] = F.cosine_similarity(
                    ci_vec.unsqueeze(0), ag_vec.unsqueeze(0)).item()
            else:
                cssv[i, c] = 0.0

    # Paper Equation 6: γᵢ = (1/M) Σⱼ (1 + Γᵢ,ⱼ)/2  ∈ [0, 1]
    gamma = np.mean((1 + cssv) / 2, axis=1)

    # Aggregation için sütun normalizasyonu
    cssv_pos = np.maximum(cssv, 0)
    for c in range(num_classes):
        s = cssv_pos[:, c].sum()
        cssv_pos[:, c] = cssv_pos[:, c] / s if s > 1e-8 else np.ones(num_clients) / num_clients

    return cssv_pos, gamma
...
class Global:
    ...
    def initialize_for_model_fusion(self, args, list_dicts_local_params,
                                    list_nums_local_data, initial_global_params,
                                    online_client_ids=None, client_gradients=None):
        fused = copy.deepcopy(list_dicts_local_params[0])
        nc    = len(list_dicts_local_params)
        total = sum(list_nums_local_data)
        weight_layer, bias_layer = _get_classifier_keys(args)
...
class Local:
    ...
    def fixmatch_train(self, args, data_labeled, data_unlabeled, init_params, r):
        ...
        # Gradient hesabi icin baslangic parametrelerini sakla (paper yontemi)
        initial_params_cpu = {k: v.float().cpu().clone() for k, v in init_params.items()}
        ...
        # Paper yontemi: gradient = (w_before - w_after) / lr
        # Sadece son katman (fc) icin hesapla — CSSV bunu kullaniyor
        weight_key = 'fc.weight' if args.model == 'resnet18' else 'classifier.weight'
        bias_key   = 'fc.weight'.replace('weight', 'bias') if args.model == 'resnet18' else 'classifier.bias'
        lr = self.optimizer.param_groups[0]['lr']
        client_grad = {}
        for k in [weight_key, bias_key]:
            if k in final_state and k in initial_params_cpu:
                client_grad[k] = (initial_params_cpu[k] - final_state[k].float().cpu()) / (lr + 1e-12)
...
def main_loop(alpha):
    ...
    for r in tqdm(range(start_round, args.num_rounds + 1), desc='Server'):
        ...
        list_params, list_nums, list_grads = [], [], []
        round_dists, round_pseudo = {}, Counter()
        ...
        for client in online:
            ...
            params, pseudo, lx, lu, client_grad = local_model.fixmatch_train(
                args, idx_labeled, idx_unlabeled, init_params, r)
            ...
            list_params.append(copy.deepcopy(params))
            list_grads.append(client_grad)  # gradient'i sakla
            round_pseudo.update(pseudo)
        ...
        # Aggregation — gradient'leri compute_cssv'ye geç
        fedavg_params = global_model.initialize_for_model_fusion(
            args, list_params, list_nums, dict_global_params,
            online_client_ids=list(online),
            client_gradients=list_grads)
        global_model.model.load_state_dict(fedavg_params)
