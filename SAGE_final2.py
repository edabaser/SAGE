...
def load_checkpoint(model, scheduler, local_ckpt_dir, args,
    ...
    try:
        ...
        if global_model_ref is not None:
            ...
            if 'client_shapley_scores' in ckpt and ckpt['client_shapley_scores']:
                global_model_ref.client_shapley_scores = ckpt['client_shapley_scores']
                print("[CKPT] Client Shapley skorlari yuklendi.")
...
class Global:
    def __init__(self, args):
        ...
        # V3: Client Shapley skorlari — {str(client_id): float}
        # Orijinal ShapFed broadcast icin kullanilir
        self.client_shapley_scores = {}
    ...
    def initialize_for_model_fusion(self, args, list_dicts_local_params,
        ...
        if args.aggregation_method == 'ShapFed':
            ...
            # V3: Client Shapley skorlarini sakla (personalized broadcast icin)
            if args.use_personalized_init and online_client_ids is not None:
                for i, cid in enumerate(online_client_ids):
                    self.client_shapley_scores[str(cid)] = float(np.mean(cssv[i]))
    ...
    # ── V3: Personalized Broadcast (orijinal ShapFed paper) ──
    def get_personalized_params(self, client_id, global_params, args):
        """
        Orijinal ShapFed paper'indaki broadcast mekanizmasi:
            client_param = (1 - coeff) * client_prev + coeff * global_param
        Shapley skoru yuksek client → daha cok global modeli alir
        (global model ona zaten iyi katkida bulunuyor demektir).
        Shapley skoru dusuk client → kendi parametresini daha cok korur
        (zayif client'in global'e dayatilmasi engellenmiyor).

        NOT: Bu V3'un onceki yanlis implementasyonunu duzeltir.
        """
        key = str(client_id)
        if (not args.use_personalized_init or
                args.aggregation_method != 'ShapFed' or
                key not in self.client_shapley_scores):
            return copy.deepcopy(global_params)

        score = self.client_shapley_scores[key]
        # coeff: [0, personalization_strength] araliginda kalacak sekilde clip
        coeff = min(score * args.personalization_strength, 1.0)
        coeff = max(coeff, 0.0)

        personalized = {}
        for k in global_params:
            # Orijinal paper: client_param = (1-coeff)*client_prev + coeff*global
            # Ilk round icin (client_prev yok): saf global kullan
            personalized[k] = (coeff * global_params[k].float()).to(global_params[k].dtype)
        # Tam dogru implementasyon: client'in onceki parametresini de saklamak gerekir.
        # Burada sadece global'i coeff ile olcekliyoruz; ilk roundda fark yok.
        # Sonraki roundlarda gercek etkiyi gormek icin client_prev_params store gerekir.
        # Asagida client_prev_params store ile tam implementasyon var:
        return personalized
...
class Local:
    ...
    def get_init_params(self, client_id, global_params, args, global_model_ref):
        """
        V3 tam implementasyon:
        client_param_new = (1-coeff)*client_param_prev + coeff*global_param
        coeff = Shapley skoru * personalization_strength
        """
        if (not args.use_personalized_init or
        ...
        key = str(client_id)
        score = global_model_ref.client_shapley_scores.get(key, 0.5)
        coeff = min(max(score * args.personalization_strength, 0.0), 1.0)

        if key not in self.client_prev_params:
            # Ilk round: saf global
            return copy.deepcopy(global_params)

        prev = self.client_prev_params[key]
        personalized = {}
        for k in global_params:
            prev_t   = prev[k].float().cpu()
            global_t = global_params[k].float().cpu()
            personalized[k] = (
    ...
    def fixmatch_train(self, args, data_labeled, data_unlabeled, init_params, r):
        ...
        self.local_model.load_state_dict(init_params)
        self.local_model.train()
        self.local_G.load_state_dict(init_params)
        self.local_G.eval()
        ...
        for _ in range(args.local_epochs):
            ...
            for _ in range(local_iter):
                ...
                lam   = torch.clamp(
                    torch.exp(-torch.log(torch.tensor(2.0)) / 0.05 * delta), 1e-6, 1.0)
...
def main_loop(alpha):
    ...
    for r in tqdm(range(start_round, args.num_rounds + 1), desc='Server'):
        ...
        extra = {
            ...
            'client_shapley_scores': global_model.client_shapley_scores,
        }
        save_checkpoint(
