"""
SAGE_v3_PersonalizedInit.py — SAGE + ShapFed Personalized Client Initialization

YENİLİK: Client'a özel personalized başlangıç parametreleri.
=========
Sorun   : SAGE'de tüm client'lar her round'da aynı global_params ile başlıyor.
          Bu client'lar arasındaki uzmanlık farkını siliyor → ShapFed ≈ FedAvg.
Çözüm   : Her client'ın önceki round parametreleri saklanır. Yeni round'da
          global model ile önceki local model Shapley skoruna göre karıştırılır.

          Shapley skoru yüksek client → daha çok kendi parametresini korusun
          (gerçekten iyi öğrenmiş, değişme)
          Shapley skoru düşük client → daha çok global modele yakın başlasın
          (kötü performans, sıfırlanmaya yakın)

          personalized = shapley_alpha * prev_local + (1-shapley_alpha) * global
          shapley_alpha = min(mean_shapley_score * personalization_strength, max_alpha)

Yeni flag: --use_personalized_init        (default False)
           --personalization_strength 2.0  (Shapley skorunu alpha'ya ölçekleme)
           --max_personalization_alpha 0.5 (max karıştırma oranı)

Checkpoint adında "PINIT" görünür.

Kullanım:
  python SAGE_v3_PersonalizedInit.py --dataset HAM10000 --model resnet18 \
    --aggregation_method ShapFed --alpha 0.1 --num_labeled 100 \
    --use_personalized_init --personalization_strength 2.0 \
...
def args_parser():
    import argparse, sys
    parser = argparse.ArgumentParser(description='SAGE v3 - PersonalizedInit', add_help=False)
    parser.add_argument('--use_personalized_init', action='store_true', default=False,
                        help='[YENİ] Shapley skoruna göre personalized client init')
    parser.add_argument('--personalization_strength', type=float, default=2.0,
                        help='Shapley → alpha ölçekleme faktörü')
    parser.add_argument('--max_personalization_alpha', type=float, default=0.5,
                        help='Max karıştırma oranı (0=pure global, 0.5=yarı-yarıya)')
...
def get_exp_name(args):
    ...
    if args.use_personalized_init:   flags.append('PINIT')  # YENİ
...
def save_checkpoint(round_num, model_state, scheduler_state, metrics_history,
                    local_ckpt_dir, args, filename='checkpoint.pt', backup_every=5):
...
class Global:
    def __init__(self, args):
        ...
        self.args = args
        # ── PERSONALIZED INIT STATE (YENİ) ────────────────────
        # {client_id: state_dict} — her client'ın önceki round parametreleri
        # {client_id: float}      — her client'ın ortalama Shapley skoru
        self.client_prev_params  = {}   # str(cid) → state_dict (CPU)
        self.client_shapley_mean = {}   # str(cid) → float

    def get_personalized_params(self, client_id, global_params, args):
        """
        Client'a özel başlangıç parametresi döner.
        Shapley skoru yüksek → daha çok kendi önceki parametresini koru.
        Shapley skoru düşük  → global modele yakın başla.
        """
        key = str(client_id)
        if (not args.use_personalized_init or
                args.aggregation_method != 'ShapFed' or
                key not in self.client_prev_params):
            return copy.deepcopy(global_params)

        prev  = self.client_prev_params[key]
        score = self.client_shapley_mean.get(key, 0.0)

        # alpha: Shapley skoru ne kadar yüksekse o kadar kendi parametresini koru
        alpha = min(score * args.personalization_strength,
                    args.max_personalization_alpha)
    ...
    def update_client_state(self, client_id, local_params, shapley_score_mean):
        """Round sonunda client'ın parametrelerini ve Shapley skorunu güncelle."""
        key = str(client_id)
    ...
    def initialize_for_model_fusion(self, args, list_dicts_local_params,
        ...
        if args.aggregation_method == 'ShapFed':
            ...
            # Client Shapley skorlarını güncelle (PINIT için)
            if args.use_personalized_init and online_client_ids is not None:
                for i, cid in enumerate(online_client_ids):
                    self.update_client_state(
                        ...
                        float(np.mean(cssv[i]))  # o client'ın ortalama Shapley skoru
                    )
        else:
            cssv = None
...
def main_loop(alpha):
    ...
    for r in tqdm(range(start_round, args.num_rounds + 1), desc='Server'):
        ...
        for client in online:
            ...
            # ── Personalized Init (YENİ) ──────────────────────
            # ShapFed + use_personalized_init aktifse client'a özel başlangıç
            client_init_params = global_model.get_personalized_params(
                client, copy.deepcopy(dict_global_params), args)

            params, pseudo, lx, lu = local_model.fixmatch_train(
                args, idx_labeled, idx_unlabeled, client_init_params, r)
