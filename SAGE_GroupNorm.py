...
class Local(object):
    def __init__(self, args):
        # KRİTİK DEĞİŞİKLİK 1: group_norm_num_groups=None yerine 16 yazıyoruz.
        # Kanal sayılarımız 64, 128, 256 olduğu için 16'ya tam ve kusursuz bölünür.
        self.local_model = ResNet(resnet_size=8, scaling=4,
                                  save_activations=False, group_norm_num_groups=16,
                                  freeze_bn=False, freeze_bn_affine=False,
                                  num_classes=args.num_classes)
        ...
        # KRİTİK DEĞİŞİKLİK 2: Eğer bir önceki mesajımda verdiğim GÜNCEL ResNet 
        # sınıfını kopyaladıysan, o sınıfın içinde zaten AdaptiveAvgPool2d var.
        # Bu yüzden aşağıdaki patch_resnet_for_ham fonksiyonlarına artık 
        # İHTİYACIN YOK, o satırları yoruma alabilirsin. 
        # (Eğer eski ResNet duruyorsa yoruma alma, çalışsın).
        ...
        # Optimizer aynı kalıyor, dışarıdan (args.lr_local_training) alacak.
        self.optimizer = torch.optim.SGD(
            self.local_model.parameters(),
            lr=args.lr_local_training, momentum=0.9, weight_decay=1e-4
        )
...
def main_loop(alpha):
    ...
    # args.num_labeled artık "Toplam" değil, "Sınıf Başına Etiketli Veri (IPC)" olarak kullanılıyor!
    ipc = args.num_labeled
    total_labeled = ipc * args.num_classes
    
    print("--> Sınıflandırma bitti. Labeled/Unlabeled ayrılıyor...")
