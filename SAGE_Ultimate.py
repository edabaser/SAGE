"""
SAGE_Ultimate.py — SAGE + Tüm İyileştirmeler Bir Arada
1. Global Model EMA Evaluation (V1)
2. Shapley Weight EMA Smoothing (V2)
3. Personalized Client Initialization (V3)
"""
...
def args_parser():
    ...
    parser = argparse.ArgumentParser(description='SAGE Ultimate', add_help=False)
    ...
    # V3: Personalized Init
    parser.add_argument('--use_personalized_init', action='store_true', default=False)
    parser.add_argument('--personalization_strength', type=float, default=2.0)
    parser.add_argument('--max_personalization_alpha', type=float, default=0.5)
    ...
    args.use_shapley_ema = extra_args.use_shapley_ema
    args.shapley_ema_decay = extra_args.shapley_ema_decay
    args.use_personalized_init = extra_args.use_personalized_init
    args.personalization_strength = extra_args.personalization_strength
    args.max_personalization_alpha = extra_args.max_personalization_alpha
...
def build_criterion(class_counts, num_classes, device, use_focal):
    if not use_focal: return nn.CrossEntropyLoss().to(device)
...
def get_exp_name(args):
    ...
    return (
        ...
        f"{flag_str}_L{args.num_labeled}_C{args.num_online_clients}_E{args.local_epochs}_"
        f"T{args.threshold}_LR{args.lr_local_training}"
    )

# ══════════════════════════════════════════════════════════════
# 4. CHECKPOINT YÖNETİMİ & SHAPLEY
# ══════════════════════════════════════════════════════════════
    ...
    if round_num % backup_every == 0 or round_num == args.num_rounds:
        try:
            ...
            if os.path.exists(local_csv):
                s3.upload_file(local_csv, args.s3_bucket, f"results/{folder_name}/metrics.csv")
            print(f"[S3] Round {round_num} → s3://{args.s3_bucket}/checkpoints/{folder_name}/")
        except Exception as e: print(f"[S3-WARN] {e}")

def load_checkpoint(model, scheduler, local_ckpt_dir, args, filename='checkpoint.pt', global_model_ref=None):
    folder_name = get_exp_name(args)
    local_path = os.path.join(local_ckpt_dir, filename)
    ...
    try:
        ckpt = torch.load(local_path, map_location='cuda' if torch.cuda.is_available() else 'cpu', weights_only=False)
...
def _get_classifier_keys(args):
    return ('fc.weight', 'fc.bias') if args.model == 'resnet18' else ('classifier.weight', 'classifier.bias')
    ...
    for _ in range(num_samples):
        ...
        for i, cid in enumerate(perm):
            ...
            for c in range(num_classes):
                cw = torch.cat([client_updates[cid][weight_layer][c].view(-1), client_updates[cid][bias_layer][c].view(-1)])
                ...
                def _sim(agg):
                    if agg is None or torch.norm(agg) == 0 or torch.norm(cw) == 0: return 0.0
...
class Global:
    def __init__(self, args):
        ...
        # V2: Shapley EMA State
        self.shapley_ema_store = {}
        self.shapley_ema_decay = getattr(args, 'shapley_ema_decay', 0.7)
        
        # V3: Personalized Init State
        self.client_prev_params  = {}
        self.client_shapley_mean = {}

    def get_personalized_params(self, client_id, global_params, args):
        key = str(client_id)
        if (not args.use_personalized_init or args.aggregation_method != 'ShapFed' or key not in self.client_prev_params):
    ...
    def update_client_state(self, client_id, local_params, shapley_score_mean):
        key = str(client_id)
        self.client_prev_params[key]  = {k: v.cpu().clone() for k, v in local_params.items()}
        self.client_shapley_mean[key] = float(shapley_score_mean)

    def initialize_for_model_fusion(self, args, list_dicts_local_params, list_nums_local_data, initial_global_params, online_client_ids=None):
        fused = copy.deepcopy(list_dicts_local_params[0])
        nc, total = len(list_dicts_local_params), sum(list_nums_local_data)
        weight_layer, bias_layer = _get_classifier_keys(args)

        if args.aggregation_method == 'ShapFed':
            cssv_raw = compute_cssv(args, list_dicts_local_params, initial_global_params)
            
            # V2: Shapley EMA Smoothing
            if args.use_shapley_ema and online_client_ids is not None:
                ...
                for i, cid in enumerate(online_client_ids):
                    ...
                    if key in self.shapley_ema_store:
                        cssv[i] = (self.shapley_ema_decay * self.shapley_ema_store[key] + (1 - self.shapley_ema_decay) * cssv_raw[i])
                    else:
                        cssv[i] = cssv_raw[i]
            ...
            # V3: Update States for PINIT
            if args.use_personalized_init and online_client_ids is not None:
                for i, cid in enumerate(online_client_ids):
                    self.update_client_state(cid, list_dicts_local_params[i], float(np.mean(cssv[i])))
        else:
            cssv, cb_w = None, np.array([n / total for n in list_nums_local_data])
        ...
        for name in list_dicts_local_params[0]:
            ...
            if args.aggregation_method == 'ShapFed' and name in (weight_layer, bias_layer):
                ...
                ft = sum(list_dicts_local_params[i][name].float() * float(cb_w[i]) for i in range(nc))
    ...
    def update_ema(self, new_params):
        if self.ema_params is None:
            ...
            for k in self.ema_params:
                self.ema_params[k] = (self.ema_decay * self.ema_params[k] + (1.0 - self.ema_decay) * new_params[k].float())
    ...
    def fedavg_eval(self, params, data_test, batch_size_test, args):
        ...
        with torch.no_grad():
            for imgs, labels in DataLoader(data_test, batch_size_test, num_workers=0, pin_memory=False):
...
class Local:
    def __init__(self, args):
        self.local_model, self.local_G = build_model(args), build_model(args)
        ...
        if args.use_cosine_lr:
            self.scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(self.optimizer, T_max=args.num_rounds, eta_min=args.lr_local_training * args.lr_min_ratio)
        else: self.scheduler = None

    def fixmatch_train(self, args, data_labeled, data_unlabeled, global_params, r):
        device = f'cuda:{args.gpu_id}'
        local_labels = [int(data_labeled.dataset.targets[data_labeled.client_dataset[i][0]]) for i in range(len(data_labeled))]
        class_counts = dict(Counter(local_labels))
        criterion = build_criterion(class_counts, args.num_classes, device, use_focal=args.use_focal_loss)

        if args.use_weighted_sampler and data_labeled.sample_weights:
            sampler = WeightedRandomSampler(weights=torch.tensor(data_labeled.sample_weights, dtype=torch.float64), num_samples=len(data_labeled.sample_weights), replacement=True)
            labeled_loader = DataLoader(data_labeled, sampler=sampler, batch_size=args.batch_size_local_labeled_fixmatch, drop_last=True, num_workers=0, pin_memory=False)
        else:
            labeled_loader = DataLoader(data_labeled, sampler=RandomSampler(data_labeled), batch_size=args.batch_size_local_labeled_fixmatch, drop_last=True, num_workers=0, pin_memory=False)
        unlabeled_loader = DataLoader(data_unlabeled, sampler=RandomSampler(data_unlabeled), batch_size=args.batch_size_local_labeled_fixmatch * args.mu, drop_last=True, num_workers=0, pin_memory=False)
        ...
        MAX_ITER = 30
        local_iter = max(min(int(data_unlabeled.client_dataset_len / args.batch_size_local_labeled_fixmatch), MAX_ITER), 5)
        total_lx, total_lu, total_batches = 0.0, 0.0, 0
        epoch_pseudo = []

        for _ in range(args.local_epochs):
            lab_iter, unlab_iter = iter(labeled_loader), iter(unlabeled_loader)
            for _ in range(local_iter):
                try: inputs_x, targets_x = next(lab_iter)
                except StopIteration: lab_iter = iter(labeled_loader); inputs_x, targets_x = next(lab_iter)
                try: inputs_u_w, inputs_u_s, _ = next(unlab_iter)
                except StopIteration: unlab_iter = iter(unlabeled_loader); inputs_u_w, inputs_u_s, _ = next(unlab_iter)

                inputs_x, inputs_u_w, inputs_u_s, targets_x = inputs_x.to(device), inputs_u_w.to(device), inputs_u_s.to(device), targets_x.to(device)
                bs = inputs_x.shape[0]

                combined = self.interleave(torch.cat((inputs_x, inputs_u_w, inputs_u_s)), 2 * args.mu + 1).to(device)
                _, logits = forward_model(self.local_model, combined)
                logits = self.de_interleave(logits, 2 * args.mu + 1)
                logits_x, (logits_u_w_loc, logits_u_s) = logits[:bs], logits[bs:].chunk(2)
                del logits

                Lx = criterion(logits_x, targets_x)
                with torch.no_grad(): _, logits_g = forward_model(self.local_G, inputs_u_w)
                ...
                if args.use_stfl:
                    ...
                    dyn_thresh = torch.clamp(args.threshold * (1.0 + args.stfl_beta * ema_norm), min=args.threshold, max=0.99)
                    mask_l, mask_g = max_pl.ge(dyn_thresh[target_l]).float(), max_pg.ge(dyn_thresh[target_g]).float()
                else:
                    mask_l, mask_g = max_pl.ge(args.threshold).float(), max_pg.ge(args.threshold).float()

                tl_oh, tg_oh = F.one_hot(target_l, args.num_classes).float(), F.one_hot(target_g, args.num_classes).float()
                ...
                final_t = torch.where(mask_l.unsqueeze(1).bool(), lam.unsqueeze(1) * tl_oh + (1 - lam).unsqueeze(1) * tg_oh, tg_oh)
                mask_valid = torch.max(mask_l, mask_g)
                logits_s_prob = torch.softmax(logits_u_s, dim=-1) + 1e-10
                Lu = (F.kl_div(logits_s_prob.log(), final_t + 1e-10, reduction='none').sum(-1) * mask_valid).mean()
                ...
                total_lx += Lx.item(); total_lu += Lu.item(); total_batches += 1
        ...
        torch.cuda.empty_cache()
        return (final_state, dict(Counter(epoch_pseudo)), total_lx / max(total_batches, 1), total_lu / max(total_batches, 1))

    def step_scheduler(self):
        if self.scheduler is not None: self.scheduler.step()
    def get_lr(self): return self.optimizer.param_groups[0]['lr']
    @staticmethod
    def interleave(x, size): s = list(x.shape); return x.reshape([-1, size] + s[1:]).transpose(0, 1).reshape([-1] + s[1:])
    @staticmethod
    def de_interleave(x, size): s = list(x.shape); return x.reshape([size, -1] + s[1:]).transpose(0, 1).reshape([-1] + s[1:])
...
class _SubsetImageFolder(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        try:
            ...
            return img, label
        except Exception: return self.__getitem__((idx + 1) % len(self.valid_indices))
...
def main_loop(args):
    ...
    logging.basicConfig(level=logging.INFO, format='%(asctime)s %(message)s', filename=f'./results/{args.dataset}/logs/{exp_name}.log', filemode='a')

    if args.dataset == 'HAM10000':
        args.num_classes = 7
        ham_mean, ham_std = [0.763, 0.545, 0.570], [0.140, 0.152, 0.169]
        transform_test = transforms.Compose([transforms.Resize((224, 224)), transforms.ToTensor(), transforms.Normalize(mean=ham_mean, std=ham_std)])
        full = ImageFolder(root=args.path_ham10000, transform=None)
        idx_tr, idx_te = train_test_split(range(len(full)), test_size=0.20, stratify=full.targets, random_state=args.seed)
        data_local_training, data_global_test = _SubsetImageFolder(full, idx_tr), _SubsetImageFolder(ImageFolder(root=args.path_ham10000, transform=transform_test), idx_te)
        print(f"[HAM10000] Train: {len(idx_tr)} | Test: {len(idx_te)}")
    else: raise ValueError(f"Desteklenmeyen dataset: {args.dataset}")
    ...
    print(f"  EMA Eval      : {'✓' if args.use_ema_eval else '✗'} (decay={args.ema_decay})")
    print(f"  Shapley EMA   : {'✓' if args.use_shapley_ema else '✗'} (decay={args.shapley_ema_decay})")
    print(f"  Personal Init : {'✓' if args.use_personalized_init else '✗'} (strength={args.personalization_strength})")
    print(f"{'='*60}\n")

    rng = np.random.RandomState(args.seed)
    list_label2indices = classify_label(data_local_training, args.num_classes)
    l_lab, l_unlab = partition_train(list_label2indices, args.num_labeled)
    c_lab, c_unlab = clients_indices(l_lab, args.num_classes, args.num_clients, args.alpha, seed=0), clients_indices(l_unlab, args.num_classes, args.num_clients, args.alpha, seed=0)
    ...
    start_round, metrics_history = load_checkpoint(global_model.model, local_model.scheduler, local_ckpt_dir, args, global_model_ref=global_model)

    idx_labeled = Indices2Dataset_labeled(data_local_training, dataset_name=args.dataset, use_medical_augment=args.use_medical_augment)
    idx_unlabeled = Indices2Dataset_unlabeled_fixmatch(data_local_training, dataset_name=args.dataset, use_medical_augment=args.use_medical_augment)

    dashboard_data, total_clients = {}, list(range(args.num_clients))
    ...
    for r in tqdm(range(start_round, args.num_rounds + 1), desc='Server'):
        ...
        list_params, list_nums, round_dists, round_pseudo = [], [], {}, Counter()

        for client in online:
            idx_labeled.load(c_lab[client]); idx_unlabeled.load(c_unlab[client])
            lbl_counts = Counter(data_local_training.targets[i] for i in c_lab[client] if i < len(data_local_training))
            round_dists[str(client)] = {str(k): v for k, v in lbl_counts.items()}
            
            print(f"\n▶ Client {client}  [LR={local_model.get_lr():.6f}]")
            print(f"    ├─ Labeled  : [{', '.join(f'{names[int(k)] if int(k)<len(names) else k}: {v}' for k, v in sorted(lbl_counts.items()))}]")
            print(f"    ├─ Unlabeled: {len(c_unlab[client])} adet")

            # V3: Personalized Init Uygula
            client_init_params = global_model.get_personalized_params(client, copy.deepcopy(dict_global_params), args)
            list_nums.append(len(c_lab[client]) + len(c_unlab[client]))
            
            params, pseudo, lx, lu = local_model.fixmatch_train(args, idx_labeled, idx_unlabeled, client_init_params, r)
            list_params.append(copy.deepcopy(params)); round_pseudo.update(pseudo)
            
            print(f"    └─ Lx: {lx:.4f} | Lu: {lu:.4f}")
            print(f"        Pseudo: [{', '.join(f'{names[int(k)] if int(k)<len(names) else k}: {v}' for k, v in sorted(pseudo.items()))}]")
            del params; torch.cuda.empty_cache()

        # V2 & V3 Aggregation ve State Güncelleme
        fedavg_params = global_model.initialize_for_model_fusion(args, list_params, list_nums, dict_global_params, online_client_ids=list(online))
        global_model.model.load_state_dict(fedavg_params)

        # V1: EMA Güncelle ve Eval Yap
        global_model.update_ema(fedavg_params)
        eval_params = ({k: v.to(fedavg_params[k].dtype) for k, v in global_model.ema_params.items()} if args.use_ema_eval and global_model.ema_params is not None else fedavg_params)
        
        acc, acsa, f1, pred_dist, per_class = global_model.fedavg_eval(copy.deepcopy(eval_params), data_global_test, args.batch_size_test, args)
        ...
        print(f"\n[Round {r:>4}/{args.num_rounds}] Acc:{acc:.4f} ACSA:{acsa:.4f} F1:{f1:.4f} ║ Best→ Acc:{b_acc:.4f} ACSA:{b_acsa:.4f} F1:{b_f1:.4f}")
        logging.info(f"Round {r} | Acc:{acc:.4f} ACSA:{acsa:.4f} F1:{f1:.4f} | Best ACSA:{b_acsa:.4f} | LR:{local_model.get_lr():.6f} | {per_class}")

        save_checkpoint(r, global_model.download_params(), local_model.scheduler.state_dict() if local_model.scheduler else {}, metrics_history, local_ckpt_dir, args, ema_params=global_model.ema_params)
        n = len(metrics_history['acc'])
        pd.DataFrame({'round': range(1, n + 1), 'acc': metrics_history['acc'], 'acsa': metrics_history['acsa'], 'f1': metrics_history['f1']}).to_csv(f'./results/{args.dataset}/{exp_name}.csv', index=False)
